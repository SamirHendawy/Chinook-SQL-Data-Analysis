...
# !\[NTI Badge](https://img.shields.io/badge/NTI-Data\_Analysis\_Track-blue)
...
# \* \*\*Automated KPIs:\*\* Engineered an automated `YOY\_Growth` View utilizing `LAG()` and `CASE` statements to flag yearly revenue as 'Peak' or 'Warning'.
...
# \* \*\*Geo-Musical Profiling:\*\* Deployed a complex CTE combining `ROW\_NUMBER()` and `GROUP\_CONCAT()` to automatically generate a readable, string-aggregated list of the Top 3 spending countries for every music genre.
...
# \* \*\*Advanced Techniques:\*\* Window Functions (`OVER`, `PARTITION BY`, `LAG`, `ROW\_NUMBER`), Common Table Expressions (CTEs), Recursive CTEs, Data Bucketing (`NTILE`, `CASE WHEN`), String Aggregation (`GROUP\_CONCAT`), Dynamic Views.
